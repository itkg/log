...
class StringFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFormat()
    {
        ...
        $this->assertEquals('UTF-8', mb_detect_encoding($this->object->format($text)));
        
        $text = "it is a new text";
        $textEncodeUtf8 = utf8_encode($text);
        $this->assertEquals($textEncodeUtf8, $this->object->format($text));
        
        $this->assertEquals($text, utf8_decode($this->object->format($text)));
